...
func main() {
	graph, startY, startX, endNode := getGraph()
	maxY := len(graph)
	maxX := len(graph[0])
	mpq := new(minPQ)
	...
	mpq.nodes = append(mpq.nodes, newNode(point{y: startY, x: startX}, 0, nil))
	...
	fmt.Printf("day12 part1: %d\n", visitedNodes[endNode].totalCost)
}

func getGraph() ([][]rune, int, int, string) {
	...
	return graph, startY, startX, endNode
}
